...
public class MultiThreadQuickSort<T> implements InternalSortMethod<T>{
	...
	@Override
	public void sort(T[] input, Comparator<T> cmp) {
		final AtomicInteger count = new AtomicInteger(1);
        pool.execute(new QuicksortRunnable<T>(input, 0, input.length - 1, count,cmp));
        try {
            synchronized (count) {
                count.wait();
            }
    ...
    private static class QuicksortRunnable<T> implements Runnable {
        ...
        public QuicksortRunnable(T[] values, int left, int right
        		, AtomicInteger count,Comparator<T> cmp) {
            this.values = values;
            this.left = left;
            this.right = right;
            this.count = count;
        ...
        private void quicksort(int pLeft, int pRight) {
            if (pLeft < pRight) {
                ...
                if (count.get() >= FALLBACK * N_THREADS) {
                    ...
                    count.getAndAdd(2);
                    pool.execute(new QuicksortRunnable<T>(values, pLeft, storeIndex - 1, count,this.cmp));
                    pool.execute(new QuicksortRunnable<T>(values, storeIndex + 1, pRight, count,this.cmp));
                }
            }
        }
        ...
        private int partition(int pLeft, int pRight) {
            ...
            for (int i = pLeft; i < pRight; i++) {
                if (cmp.compare(values[i], pivotValue) < 0) {
                    swap(i, storeIndex);
                    storeIndex++;
                }
            }
