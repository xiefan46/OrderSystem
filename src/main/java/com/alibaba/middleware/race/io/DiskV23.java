...
public class DiskV23 {
	...
	private final InfoManagerV13 buyerManager;
	
	private final InfoManagerV13 goodManager;
	...
	public DiskV23(int diskId,String storeRootPath,String name,ExecutorService executor) throws Exception
	{
		this.diskId = diskId;
		...
		this.buyerManager = new InfoManagerV13(this.storeRootPath+"buyer_manager_dir/", "buyerid");
		this.goodManager = new InfoManagerV13(this.storeRootPath+"good_manager_dir/", "goodid");
	...
	public LightRow queryBuyer(StringSecondIndexV13 secondIndex)
	{
		int lockid = -1;
		try {
			lockid = getlock(buyer_lock);
			buyerParsers[lockid].seek(secondIndex.getOffset());
	...
	public LightRow queryGood(StringSecondIndexV13 secondIndex)
	{
		int lockid = -1;
		try {
			lockid = getlock(good_lock);
			goodParsers[lockid].seek(secondIndex.getOffset());
	...
	public InfoManagerV13 getBuyerManager() {
		return buyerManager;
	}

	public InfoManagerV13 getGoodManager() {
		return goodManager;
	}
